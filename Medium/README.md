...
在模 $k$ 的世界裡，加法和乘法是**確定性**的。

* **如果** $A \equiv A' \pmod k$，**則** $A \times B \equiv A' \times B \pmod k$。
* **如果** $A \equiv A' \pmod k$，**則** $A + B \equiv A' + B \pmod k$。

##### 2. 全 $1$ 數的餘數是線性的確定遞推關係

我們計算 $\mathbf{r}_{L}$ 的遞推公式是：
$$\mathbf{r}_{\text{new}} = (\mathbf{r}_{\text{old}} \times 10 + 1) \bmod k$$

這是一個標準的**一階線性遞推關係**，形式為 $\mathbf{r}_{\text{new}} = (A \cdot \mathbf{r}_{\text{old}} + B) \bmod k$（其中 $A=10, B=1$）。

###### 最終的證明步驟

假設我們在兩個不同的長度 $L_1$ 和 $L_2$ ($L_1 < L_2$) 處得到了相同的餘數：

$$\mathbf{r}_{L_1} = \mathbf{r}_{L_2} = R^*$$

1.  **計算下一步：** 我們計算 $\mathbf{r}_{L_1+1}$ 和 $\mathbf{r}_{L_2+1}$。
2.  **確定性：** 由於兩個輸入都是 $R^*$，且遞推公式 $f(r)=(r \times 10 + 1) \bmod k$ 是確定的，所以：
    $$\mathbf{r}_{L_1+1} = (R^* \times 10 + 1) \bmod k$$
    $$\mathbf{r}_{L_2+1} = (R^* \times 10 + 1) \bmod k$$
3.  **結論：** $\mathbf{r}_{L_1+1} = \mathbf{r}_{L_2+1}$。

既然第 $L_1+1$ 步和第 $L_2+1$ 步的餘數相等，我們可以用同樣的邏輯證明 $\mathbf{r}_{L_1+2} = \mathbf{r}_{L_2+2}$，依此類推。

這個序列從 $L_1$ 到 $L_2-1$ 的所有餘數，將會被 $\mathbf{r}_{L_2}$ 到 $\mathbf{r}_{2L_2-L_1-1}$ 依序重複。
...
在任何**確定性**的遞推關係中（不論是在數論、動態規劃還是有限狀態機中），**一旦狀態（餘數）重複，其未來的序列就完全鎖定，無法跳出循環，也無法產生新的餘數。** 這就是為什麼這個算法可以只檢查循環的原因。
